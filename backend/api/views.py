        ...
        try:
            with connection.cursor() as cursor:
                ...
                for record in video_records:
                    ...
                    # 构建完整路径
                    # video_path 可能是 "videos/3/xxx.mp4" 或 "3/xxx.mp4"
                    # 如果以 "videos/" 开头，需要去掉，因为已经在 videos_dir 中了
                    if video_path:
                        # 去掉开头的 "videos/"（如果存在）
                    ...
                    if full_path.exists() and full_path.is_file():
                        ...
                        # DEBUG: 输出原始数据库数据
                        print(f"DEBUG: 视频记录 - ID: {video_id}, 名称: {video_name}")
                        print(f"DEBUG: 数据库字段 - file_size: {record.get('file_size')}, video_duration: {record.get('video_duration')}, total_frames: {record.get('total_frames')}")
                        print(f"DEBUG: 文件路径: {full_path}")
                        print(f"DEBUG: 文件是否存在: {full_path.exists()}")
