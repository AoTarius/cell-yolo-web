...
watch(compareTaskIdA, () => {
  const stillValid = availableTaskBOptions.value.some((task) => task.task_id === compareTaskIdB.value)
  if (!stillValid) {
    compareTaskIdB.value = ''
  }
})
...
watch(
  ...
  async (username, oldUsername) => {
    // 用户登录态异步恢复后，重新拉取一次模型/任务，避免页面误显示“暂无模型”
            ...
            <div v-else class="model-items">
              ...
              >
                ...
                <div class="model-info">
                  <p class="model-name">{{ getDisplayModelName(model.name) }}</p>
...
:global(:root:not(.dark)) .model-compare-card {
...
:global(:root:not(.dark)) .model-compare-card h3 {
  color: var(--text-primary-light);
}
